 ...
 *
 * Top-level menu widget classes come from UV3DSimulatorAssetRegistry and are created automatically.
 * Blueprint subclasses normally only provide layout/button wiring; Set*Widget remains an override path.
 */
UCLASS(Blueprintable, BlueprintType)
class V3DSIMULATOR_API AMainGameMode : public AGameModeBase
{
    GENERATED_BODY()
...
public:
    ...
    /** Validates and resolves an exact world-folder key from the current index of .v3d files under Worlds. */
    UFUNCTION(BlueprintCallable, Category="Start World|Data")
...
private:
    ...
    const FV3DWorldLaunchProfile* FindWorldLaunchProfile(const FString& WorldFolderName) const;
