...
#include "Model/V3DMaterialAssetReferences.h"
...
class UProjectSourceModelBuilder;
class UGameSettings;
class UPostProcessComponent;
class AV3DSimulatorGameplayGameModeBase;
class UWorld;
...
struct V3DSIMULATOR_API FToolbarItem
{
    GENERATED_BODY()
...
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FProjectBuildProgress, float, Progress);
DECLARE_DYNAMIC_MULTICAST_DELEGATE_TwoParams(FProjectBuildCompleted, bool, bSuccess, const FString&, Message);

UCLASS(Blueprintable, BlueprintType)
class V3DSIMULATOR_API UGameManagerSubSystem : public UGameInstanceSubsystem
{
    GENERATED_BODY()
...
public:
    ...
    bool IsActiveGameMode(const AV3DSimulatorGameplayGameModeBase* GameMode) const;
     ...
     */
    const FV3DMaterialAssetReferences& GetMaterialDefaultReferences();
    ...
    void StartGameplaySession(AV3DSimulatorGameplayGameModeBase* InGameMode);
    ...
    void StopGameplaySession(
        ...
        const AV3DSimulatorGameplayGameModeBase* RequestingGameMode = nullptr);
    void UpdateGameManager(float DeltaSeconds);
    void ApplyGameModeConfig(const AV3DSimulatorGameplayGameModeBase* InGameMode);
    ...
    /**
     * Stores a direct child key beneath WorldsRoot. Empty input deliberately clears the hand-off;
    ...
    /** Creates Projects/<Name>/config.json and Projects/<Name>/resources/. */
    UFUNCTION(BlueprintCallable, Category="Game|Projects")
    bool CreateProject(const FString& ProjectName, EV3DSimulatorProjectType ProjectType);

    /** Scans Projects/<Project>/resources recursively and publishes either a World .v3d or Resources/<Project>.v3d. */
    UFUNCTION(BlueprintCallable, Category="Game|Projects")
    ...
    /** Returns the top-level built resource-pack repository: V3DSimulator/Resources. */
    UFUNCTION(BlueprintPure, Category="Game|Projects")
    static FString GetResourcesRootPath();

    /** Returns valid Projects/<Folder>/config.json projects and ensures each resources/ folder exists. */
    UFUNCTION(BlueprintCallable, Category="Game|Projects")
    void GetProjectSummaries(TArray<FV3DSimulatorProjectSummary>& OutProjects) const;
    ...
    UFUNCTION(BlueprintCallable, Category="Game|Projects")
    bool SetProjectTypeByName(const FString& ProjectName, EV3DSimulatorProjectType ProjectType);

    UFUNCTION(BlueprintCallable, Category="Game|Projects")
    bool SetWorldProjectAssetsAllowedByName(const FString& ProjectName, bool bAllowed);

    /** Atomically builds the active project into .v3d (World) or .v3d (asset project). */
    UFUNCTION(BlueprintCallable, Category="Game|Project Build")
    void BuildActiveProjectArchive();

    /** Includes project validation/indexing before the source-model bake flag becomes true. */
    UFUNCTION(BlueprintPure, Category="Game|Project Build")
    bool IsProjectBuildInProgress() const
    {
        return ProjectBuild.bInProgress || ProjectBuild.bCommitInFlight || !ProjectBuild.Root.IsEmpty();
    }

    UFUNCTION(BlueprintPure, Category="Game|Project Build")
    float GetProjectBuildProgress() const { return ProjectBuild.Progress; }

    UFUNCTION(BlueprintPure, Category="Game|Project Build")
    int32 GetProjectBuildTotalModels() const { return ProjectBuild.TotalModels; }

    UFUNCTION(BlueprintPure, Category="Game|Project Build")
    int32 GetProjectBuildCompletedModels() const { return ProjectBuild.CompletedModels; }
    ...
    /** Returns true when initial .v3d model ranges and the player replacement are ready. */
    UFUNCTION(BlueprintPure, Category="Game|World")
    ...
    UPROPERTY(BlueprintAssignable, Category="Game|Project Build")
    FProjectBuildProgress OnProjectBuildProgress;

    UPROPERTY(BlueprintAssignable, Category="Game|Project Build")
    FProjectBuildCompleted OnProjectBuildCompleted;
...
private:
    ...
    UPROPERTY(Transient)
    TWeakObjectPtr<AV3DSimulatorGameplayGameModeBase> ConfigGameMode;
    ...
    struct FProjectBuildState
    {
        ...
        bool bCommitInFlight = false;
        uint64 Generation = 0;
        TSharedPtr<TAtomic<bool>, ESPMode::ThreadSafe> Cancellation;
        float Progress = 0.0f;
        int32 TotalModels = 0;
        int32 CompletedModels = 0;
        int32 FailedModels = 0;
        int32 NextModelIndex = 0;
        TArray<FModelDefinition> PendingModels;
        TArray<FGWorldBuildModel> CompletedModelsData;
        TArray<FSoundDefinition> PendingSounds;
        TMap<FGuid, FString> PendingSoundJson;

        void ClearPayload()
        {
            PendingModels.Reset();
            CompletedModelsData.Reset();
            PendingSounds.Reset();
            PendingSoundJson.Reset();
            ConfigJson.Reset();
        }
        ...
        void Reset()
        {
            ...
            Root.Reset();
            Name.Reset();
            Type = EV3DSimulatorProjectType::World;
            bInProgress = false;
            bCommitInFlight = false;
            Cancellation.Reset();
            Progress = 0.0f;
            TotalModels = 0;
            CompletedModels = 0;
    ...
    FProjectBuildState ProjectBuild;
    bool bWorldStartupContinued = false;
    UPROPERTY(Transient)
    TObjectPtr<UProjectSourceModelBuilder> ActiveProjectBuildTask;
    ...
    FTimerHandle ProjectBuildProgressTimerHandle;

    void StartGameplaySessionInternal(AActor* InOwnerActor, const AV3DSimulatorGameplayGameModeBase* InConfigGameMode);
    ...
    void StartNextProjectBuildModel();
    void RefreshProjectBuildProgress();
    void HandleProjectBuildModelFinished(UProjectSourceModelBuilder* BuildTask, bool bSuccess);
    void FinishProjectBuild();
    void CompleteProjectArchiveBuild(bool bSuccess, const FString& ArchivePath, const FString& Error);
