...
struct FModelData
{
    ...
    /** Build-derived model center stored in the .v3d directory, never in author JSON. */
    UPROPERTY()
    FVector Center = FVector::ZeroVector;

    /** Build-derived full model size stored in the .v3d directory, never in author JSON. */
    UPROPERTY()
    FVector Size = FVector::ZeroVector;
...
struct FModelMeshData
{
    ...
    /** Unscaled local-space mesh half size; the .v3d metadata member also stores full Size. */
    UPROPERTY()
...
/** Terminal result from one mesh-group or water-group .v3d streaming pass. */
USTRUCT(BlueprintType)
