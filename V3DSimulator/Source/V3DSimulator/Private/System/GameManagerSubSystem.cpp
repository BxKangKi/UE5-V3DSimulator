...
#include "System/ProjectSourceModelBuilder.h"
...
const FV3DMaterialAssetReferences& UGameManagerSubSystem::GetMaterialDefaultReferences()
{
    ...
    static const FV3DMaterialAssetReferences EmptyReferences;
...
bool UGameManagerSubSystem::ResolveMaterialDefaultAsset()
{
    ...
    const bool bResolveCallSucceeded = Registry->V3DMaterials.Resolve(
        NewCache->References,
        Failures);
    bMaterialDefaultAssetResolved = true;
    ...
    UE_LOG(LogTemp, Display,
        ...
        Registry->V3DMaterials.NumConfiguredReferences(),
    ...
    const bool bRequireExistingDirectory)
{
    ...
    {
        const FString VirtualRoot = FPaths::Combine(V3DSimulatorPaths::WorldsRoot(), OutNormalized);
...
void UGameManagerSubSystem::ApplyGameModeConfig(const AV3DSimulatorGameplayGameModeBase* InConfigGameMode)
{
...
bool UGameManagerSubSystem::IsActiveGameMode(const AV3DSimulatorGameplayGameModeBase* GameMode) const
{
    return ConfigGameMode.Get() == GameMode && SessionOwner.Get() == GameMode;
}

void UGameManagerSubSystem::StartGameplaySession(AV3DSimulatorGameplayGameModeBase* InGameMode)
{
...
void UGameManagerSubSystem::StartClientGameplaySession(APlayerController* InOwnerController)
{
    ...
    const AV3DSimulatorGameplayGameModeBase* Defaults =
        GetDefault<AV3DSimulatorGameplayGameModeBase>();
    ...
    if (const UWorld* World = InOwnerController->GetWorld())
    {
        if (const AGameStateBase* GameState = World->GetGameState())
        {
            ...
            if (IsValid(ReplicatedGameModeClass)
                && ReplicatedGameModeClass->IsChildOf(AV3DSimulatorGameplayGameModeBase::StaticClass()))
            {
                if (const AV3DSimulatorGameplayGameModeBase* ReplicatedDefaults =
                    Cast<AV3DSimulatorGameplayGameModeBase>(ReplicatedGameModeClass->GetDefaultObject()))
                {
...
void UGameManagerSubSystem::StartGameplaySessionInternal(
    AActor* InOwnerActor,
    const AV3DSimulatorGameplayGameModeBase* InConfigGameMode)
{
    ...
    ConfigGameMode = Cast<AV3DSimulatorGameplayGameModeBase>(InOwnerActor);
    ApplyGameModeConfig(InConfigGameMode);

    // World environment is a rendering prerequisite and intentionally starts before .v3d I/O.
    SpawnWorldEnvManager();
            ...
            [WeakThis, StartupOwner](const bool bSuccess, const FString& Error)
            {
                ...
                if (!ReadyDatabase || !ReadyDatabase->IsBuiltWorld())
                {
                    StrongThis->FailWorldStartup(
                        TEXT("World startup failed: runtime uses Worlds/*.v3d only. Build the project first from the Projects UI in MainWorld."));
                    return;
                }
...
void UGameManagerSubSystem::ContinueWorldStartupAfterDatabase()
{
    ...
    {
        FailWorldStartup(
            TEXT("World startup failed: no verified .v3d archive is open."));
        return;
    }
...
void UGameManagerSubSystem::StopGameplaySession(
    ...
    const AV3DSimulatorGameplayGameModeBase* RequestingGameMode)
{
...
void UGameManagerSubSystem::ValidatePostLoadGameplayLifecycle(UWorld* LoadedWorld)
{
    ...
    if (Cast<AV3DSimulatorGameplayGameModeBase>(LoadedWorld->GetAuthGameMode()))
    {
        return;
    }
    ...
    // A world that resolves to a real .v3d but does not use the gameplay GameMode classes is a
    ...
    {
        UE_LOG(LogTemp, Error,
            TEXT("Gameplay world '%s' resolved .v3d '%s' but its active GameMode is not derived from AV3DSimulatorGameplayGameModeBase. Assign SingleplayGameMode or MultiplayGameMode in World Settings / travel override."),
...
void UGameManagerSubSystem::InitializeRuntimeWorldState()
{
    ...
    // Loading UI, world data, world rendering, water, and .v3d streaming now start from one owner.
    ShowLoadingWidget();
    LoadWorldData();
}
...
void UGameManagerSubSystem::LoadWorldData()
{
    ...
    {
        FailWorldStartup(TEXT("World startup failed: no .v3d reader is available for config.json."));
        return;
    }
    TWeakObjectPtr<UGameManagerSubSystem> WeakThis(this);
    const bool bQueued = FSafeFileIO::RunTrackedWorker([WeakThis, ArchiveReader, RequestedRoot, StartupOwner]()
    {
        ...
        {
            Config = FSafeFileIO::ParseJsonText(ConfigText, TEXT(".v3d/config.json"), Limits);
        }
        else
        {
        ...
        FSafeFileIO::DispatchTrackedGameThread(
            [WeakThis, RequestedRoot, StartupOwner, Config = MoveTemp(Config)]() mutable
            {
                        ...
                        FString StateError) mutable
                {
                    ...
                    {
                        Manager->FailWorldStartup(FString::Printf(
                            TEXT("World startup failed: canonical config.json could not be read. Path=%s Reason=%s"),
                            *Config.Path, *Config.Error));
                        return;
                    }

                    FV3DSimulatorProjectConfig RuntimeProjectConfig;
                    FString ProjectConfigError;
                    if (!V3DSimulatorProjectConfig::Parse(
                            Config.JsonObject,
                            FPaths::GetCleanFilename(RequestedRoot),
                            RuntimeProjectConfig,
                            ProjectConfigError)
                        || RuntimeProjectConfig.ProjectType != EV3DSimulatorProjectType::World)
                    {
                        Manager->FailWorldStartup(FString::Printf(
                            TEXT("World startup failed: archive config.json is not a canonical World project config. Reason=%s"),
                            *ProjectConfigError));
                        return;
                    }
                    ...
                    if (!UWorldData::DeserializeData(
                            Manager->ActiveWorldData, Config.JsonObject))
                    {
                        Manager->FailWorldStartup(
                            TEXT("World startup failed: canonical config.json could not be applied."));
                        return;
                    }
...
void UGameManagerSubSystem::StartGameplayWorldStreaming(const FString& InWorldRoot, const FString& InInitialPlayerName)
{
    ...
    // StartWorldStreaming deliberately refuses every non-.v3d source. Convert that
        ...
        || !StreamSubSystem->IsActiveForWorld(World))
    {
        StreamSubSystem = nullptr;
        FailWorldStartup(
            TEXT("World startup failed: the verified .v3d streaming session could not be started."));
    }
}
...
void UGameManagerSubSystem::ClearTransientRuntimeReferences()
{
    ...
    ProjectBuild.PendingModels.Empty();
    ProjectBuild.CompletedModelsData.Empty();
    ProjectBuild.PendingSounds.Empty();
    ProjectBuild.PendingSoundJson.Empty();
    ActiveProjectBuildTask = nullptr;
    ProjectBuild.bInProgress = false;
    ProjectBuild.bCommitInFlight = false;
    bWorldStartupContinued = false;
    ProjectBuild.TotalModels = 0;
    ProjectBuild.CompletedModels = 0;
    ProjectBuild.FailedModels = 0;
    ProjectBuild.NextModelIndex = 0;
...
FString UGameManagerSubSystem::GetWorldRootPath() const
{
    ...
    return FPaths::Combine(V3DSimulatorPaths::WorldsRoot(), WorldName);
}
...
FString UGameManagerSubSystem::GetAssetDisplayName(const FString& ModelReference) const
{
    ...
    // Invalid references are displayed as opaque identifiers. Runtime UI never probes project authoring files.
...
{
    return V3DSimulatorProjectWorkspace::ProjectsRoot();
}

FString UGameManagerSubSystem::GetResourcesRootPath()
{
    V3DSimulatorProjectWorkspace::EnsureWorkspaceRoots();
    return V3DSimulatorPaths::ResourcesRoot();
}

void UGameManagerSubSystem::GetProjectSummaries(
    TArray<FV3DSimulatorProjectSummary>& OutProjects) const
{
    check(IsInGameThread());
    FString Error;
    if (!V3DSimulatorProjectRepository::ListProjects(OutProjects, Error) && !Error.IsEmpty())
    {
        UE_LOG(LogTemp, Warning, TEXT("Project listing failed: %s"), *Error);
    }
}

bool UGameManagerSubSystem::CreateProject(
    ...
    const EV3DSimulatorProjectType ProjectType)
{
    ...
    FString Error;
    const bool bCreated = V3DSimulatorProjectRepository::CreateProject(ProjectName, ProjectType, Error);
    LastSaveMessage = bCreated
        ? FString::Printf(TEXT("%s project created: %s"), *V3DSimulatorProjectTypes::ToString(ProjectType), *ProjectName)
        : Error;
    NotifyStateChanged();
    return bCreated;
}
...
{
    OutProjectType = EV3DSimulatorProjectType::World;
    bOutAllowProjectAssets = false;
    OutDisplayName.Reset();
    FV3DSimulatorProjectConfig Config;
    FString Error;
    if (!V3DSimulatorProjectRepository::LoadProject(ProjectName, Config, nullptr, Error)) return false;
    OutProjectType = Config.ProjectType;
    bOutAllowProjectAssets = Config.bAllowProjectAssets;
    OutDisplayName = Config.GetDisplayName(ProjectName);
    return true;
}
    ...
    const EV3DSimulatorProjectType ProjectType)
{
    ...
    FString Error;
    const bool bChanged = V3DSimulatorProjectRepository::SetProjectType(ProjectName, ProjectType, Error);
    LastSaveMessage = bChanged
        ? FString::Printf(TEXT("Project type changed to %s. Rebuild required."), *V3DSimulatorProjectTypes::ToString(ProjectType))
        : Error;
    NotifyStateChanged();
    return bChanged;
}

bool UGameManagerSubSystem::SetWorldProjectAssetsAllowedByName(
    const FString& ProjectName,
    const bool bAllowed)
{
    check(IsInGameThread());
    if (IsProjectBuildInProgress()) return false;
    FString Error;
    const bool bChanged = V3DSimulatorProjectRepository::SetWorldProjectAssetsAllowed(ProjectName, bAllowed, Error);
    LastSaveMessage = bChanged
        ? FString::Printf(TEXT("Project assets are now %s for this world. Rebuild required."), bAllowed ? TEXT("enabled") : TEXT("disabled"))
        : Error;
    NotifyStateChanged();
    return bChanged;
}
...
bool UGameManagerSubSystem::BuildProjectByName(const FString& ProjectName)
{
    ...
    if (!V3DSimulatorProjectWorkspace::NormalizeProjectName(ProjectName, SafeName)
        || ProjectBuild.bInProgress || !ProjectBuild.Root.IsEmpty())
    {
        LastSaveMessage = (ProjectBuild.bInProgress || !ProjectBuild.Root.IsEmpty())
    ...
    const FString ProjectRoot = V3DSimulatorProjectWorkspace::ProjectRoot(SafeName);
    const FString ProjectResourcesRoot = V3DSimulatorProjectWorkspace::ProjectResourcesRoot(SafeName);
    const FString ConfigPath = V3DSimulatorProjectWorkspace::ConfigPath(SafeName);
    if (ProjectRoot.IsEmpty() || ProjectResourcesRoot.IsEmpty()
        || !IFileManager::Get().DirectoryExists(*ProjectRoot)
        || !IFileManager::Get().FileExists(*ConfigPath)
        || !V3DSimulatorProjectWorkspace::EnsureProjectDirectories(SafeName)
        || !IFileManager::Get().DirectoryExists(*ProjectResourcesRoot))
    {
        LastSaveMessage = TEXT("The project requires Projects/<Project>/config.json and Projects/<Project>/resources/.");
        NotifyStateChanged();
        return false;
    }
    ...
    ProjectBuild.Root = ProjectRoot;
    ProjectBuild.Name = SafeName;
    ProjectBuild.Type = EV3DSimulatorProjectType::World;
    ProjectBuild.Progress = 0.01f;
    LastSaveMessage = FString::Printf(TEXT("Validating project configuration: %s"), *SafeName);
    OnProjectBuildProgress.Broadcast(ProjectBuild.Progress);
    NotifyStateChanged();
        ...
        [WeakThis, ProjectRoot, SafeName, ConfigPath]()
    {
        ...
        if (V3DSimulatorProjectConfig::Load(
                ConfigPath, SafeName, ProjectConfig, &Canonical, ValidationError)
            && Canonical.IsValid())
        {
            ...
            {
                ValidationError = TEXT("Project config.json could not be serialized for the archive.");
            }
        }
        ...
        {
            UGameManagerSubSystem* StrongThis = WeakThis.Get();
            if (!IsValid(StrongThis) || StrongThis->ProjectBuild.Root != ProjectRoot
                || StrongThis->ProjectBuild.Name != SafeName)
            {
                return;
            }
            if (!ValidationError.IsEmpty())
            {
                StrongThis->ProjectBuild.Reset();
                StrongThis->LastSaveMessage = ValidationError;
                StrongThis->OnProjectBuildCompleted.Broadcast(false, StrongThis->LastSaveMessage);
            ...
            StrongThis->ProjectBuild.Type = ProjectConfig.ProjectType;
            StrongThis->ProjectBuild.ConfigJson = MoveTemp(ConfigJson);
            StrongThis->ProjectBuild.Progress = FMath::Max(StrongThis->ProjectBuild.Progress, 0.03f);
            StrongThis->OnProjectBuildProgress.Broadcast(StrongThis->ProjectBuild.Progress);
            FV3DRuntimeSafety::ResetRecoverableFailures();
            StrongThis->LastSaveMessage = FString::Printf(
                TEXT("Indexing %s project assets: %s"),
                *V3DSimulatorProjectTypes::ToString(ProjectConfig.ProjectType), *SafeName);
            StrongThis->NotifyStateChanged();
                    ...
                    [WeakThis, ProjectRoot, SafeName](const bool bReady, const FString& Error)
            {
                ...
                if (!IsValid(InnerThis) || InnerThis->ProjectBuild.Root != ProjectRoot
                    || InnerThis->ProjectBuild.Name != SafeName)
                {
                    return;
                }
                if (!bReady)
                {
                    InnerThis->ProjectBuild.Reset();
                    InnerThis->LastSaveMessage = FString::Printf(TEXT("Project validation failed: %s"), *Error);
                    InnerThis->OnProjectBuildCompleted.Broadcast(false, InnerThis->LastSaveMessage);
                    InnerThis->NotifyStateChanged();
                    return;
                }
                InnerThis->ProjectBuild.Progress = FMath::Max(InnerThis->ProjectBuild.Progress, 0.05f);
                InnerThis->OnProjectBuildProgress.Broadcast(InnerThis->ProjectBuild.Progress);
                InnerThis->BuildActiveProjectArchive();
            }));
        });
    });
...
void UGameManagerSubSystem::BuildActiveProjectArchive()
{
    UWorld* World = GetWorld();
    if (!World || World->GetNetMode() == NM_Client)
    {
        ...
        LastSaveMessage = Message;
        OnProjectBuildCompleted.Broadcast(false, Message);
        ProjectBuild.Reset();
        NotifyStateChanged();
        return;
    }

    if (ProjectBuild.bInProgress)
    {
    ...
    ProjectBuild.bInProgress = true;
    ++ProjectBuild.Generation;
    ProjectBuild.Cancellation = MakeShared<TAtomic<bool>, ESPMode::ThreadSafe>(false);
    ProjectBuild.bCommitInFlight = false;
    ProjectBuild.Progress = FMath::Max(ProjectBuild.Progress, 0.05f);
    ProjectBuild.TotalModels = 0;
    ProjectBuild.CompletedModels = 0;
    ProjectBuild.FailedModels = 0;
    ProjectBuild.NextModelIndex = 0;
    ...
    ModelDatabase->GetSoundDefinitions(ProjectBuild.PendingSounds);
    ProjectBuild.PendingSoundJson.Reserve(ProjectBuild.PendingSounds.Num());
    for (const FSoundDefinition& Sound : ProjectBuild.PendingSounds)
    {
        FString SoundJson;
        FString SoundError;
        if (!ModelDatabase->GetSoundDefinitionJson(Sound.UUID, SoundJson, &SoundError) || SoundJson.IsEmpty())
        {
            CompleteProjectArchiveBuild(false, FString(), FString::Printf(
                TEXT("Sound definition could not be prepared. Sound=%s Reason=%s"),
                *Sound.DisplayName, *SoundError));
            return;
        }
        ProjectBuild.PendingSoundJson.Add(Sound.UUID, MoveTemp(SoundJson));
    }
    ProjectBuild.PendingModels.Reserve(Definitions.Num());
    ...
    for (const FModelDefinition& Definition : Definitions)
    {
        ...
        if (!V3DSimulatorProjectBuildPolicy::AllowsModelType(ProjectBuild.Type, Definition.ModelType))
        {
            TypeMismatchError = FString::Printf(
                TEXT("ProjectType=%s does not allow model '%s' with ModelType=%s."),
                *V3DSimulatorProjectTypes::ToString(ProjectBuild.Type),
                *Definition.DisplayName,
                *V3DSimulatorModelTypes::ToString(Definition.ModelType));
            break;
        }
        ProjectBuild.PendingModels.Add(Definition);
    }

    if (!TypeMismatchError.IsEmpty())
    {
        CompleteProjectArchiveBuild(false, FString(), TypeMismatchError);
        return;
    }

    ProjectBuild.TotalModels = ProjectBuild.PendingModels.Num();
    ...
    LastSaveMessage = FString::Printf(
        ...
        *V3DSimulatorProjectTypes::ToString(ProjectBuild.Type),
        ProjectBuild.TotalModels, ProjectBuild.PendingSounds.Num());
    NotifyStateChanged();

    if (ProjectBuild.TotalModels > 0)
    {
        World->GetTimerManager().SetTimer(
            ProjectBuildProgressTimerHandle,
            this,
            &UGameManagerSubSystem::RefreshProjectBuildProgress,
            0.05f,
            true);
        StartNextProjectBuildModel();
    }
    else if (!ProjectBuild.PendingSounds.IsEmpty())
    {
        FinishProjectBuild();
    }
    else
    {
        CompleteProjectArchiveBuild(false, FString(),
            TEXT("recursive project scan produced zero buildable Model or Sound assets"));
    }
}

void UGameManagerSubSystem::StartNextProjectBuildModel()
{
    if (!ProjectBuild.bInProgress)
    {
        return;
    ...
    while (ProjectBuild.NextModelIndex < ProjectBuild.PendingModels.Num())
    {
        const FModelDefinition Definition = ProjectBuild.PendingModels[ProjectBuild.NextModelIndex++];
        const FString ModelPath = GlbValidation::NormalizePath(Definition.GlbPath);
        if (ModelPath.IsEmpty() || !IFileManager::Get().FileExists(*ModelPath))
        {
            ++ProjectBuild.CompletedModels;
            ++ProjectBuild.FailedModels;
            RefreshProjectBuildProgress();
            continue;
        }
        ...
        {
            UE_LOG(LogTemp, Error,
                TEXT("Project source JSON unavailable. GLB=%s Reason=%s"),
        ...
        UProjectSourceModelBuilder* BuildTask =
            NewObject<UProjectSourceModelBuilder>(this, NAME_None, RF_Transient);
        ...
        ActiveProjectBuildTask = BuildTask;
        LastSaveMessage = FString::Printf(
            TEXT("Building model %d/%d: %s"),
            ProjectBuild.CompletedModels + 1,
            ProjectBuild.TotalModels,
            *FPaths::GetCleanFilename(ModelPath));
        NotifyStateChanged();
        UE_LOG(LogTemp, Display,
            TEXT("Building source GLB %d/%d: %s"),
            ProjectBuild.CompletedModels + 1, ProjectBuild.TotalModels, *ModelPath);
        BuildTask->OnFinished.AddUObject(
            this,
            &UGameManagerSubSystem::HandleProjectBuildModelFinished);
...
void UGameManagerSubSystem::RefreshProjectBuildProgress()
{
    if (!ProjectBuild.bInProgress)
    {
        return;
    ...
    const float ActiveModelProgress = IsValid(ActiveProjectBuildTask)
        ? FMath::Clamp(ActiveProjectBuildTask->GetProgress(), 0.0f, 1.0f)
        : 0.0f;
    const float ModelProgress = ProjectBuild.TotalModels > 0
        ? FMath::Clamp(
            (static_cast<float>(ProjectBuild.CompletedModels) + ActiveModelProgress) /
                static_cast<float>(ProjectBuild.TotalModels),
            0.0f,
            1.0f)
        : 1.0f;
    ...
    const float SafeProgress = FMath::Max(ProjectBuild.Progress, CalculatedProgress);
    if (!FMath::IsNearlyEqual(SafeProgress, ProjectBuild.Progress, KINDA_SMALL_NUMBER))
    {
        ProjectBuild.Progress = SafeProgress;
        OnProjectBuildProgress.Broadcast(ProjectBuild.Progress);
    }
}

void UGameManagerSubSystem::HandleProjectBuildModelFinished(
    UProjectSourceModelBuilder* BuildTask,
    bool bSuccess)
{
    if (!ProjectBuild.bInProgress
        || !IsValid(BuildTask)
        || BuildTask != ActiveProjectBuildTask.Get())
    {
        return;
    }
    ...
    if (bSuccess)
    {
            ...
            || Built.SourceFileSize <= 0)
        {
            bSuccess = false;
            UE_LOG(LogTemp, Error,
                TEXT("Project source build result rejected. Reason=%s"),
                ValidationError.IsEmpty()
                    ? TEXT("incomplete build payload") : *ValidationError);
        }
        else
        {
            ProjectBuild.CompletedModelsData.Add(MoveTemp(Built));
        }
    }
    else
    {
        UE_LOG(LogTemp, Error,
            TEXT("Project source model failed. GLB=%s Reason=%s"),
    ...
    {
        World->GetTimerManager().SetTimerForNextTick(
            this, &UGameManagerSubSystem::StartNextProjectBuildModel);
    }
    else
    {
        StartNextProjectBuildModel();
...
void UGameManagerSubSystem::FinishProjectBuild()
{
    if (!ProjectBuild.bInProgress || ProjectBuild.bCommitInFlight)
    {
        return;
    }

    if (ProjectBuild.FailedModels > 0
        || ProjectBuild.CompletedModelsData.Num() != ProjectBuild.TotalModels)
    {
        CompleteProjectArchiveBuild(false, FString(),
            FString::Printf(TEXT("model build failed for %d/%d models"),
                ProjectBuild.FailedModels, ProjectBuild.TotalModels));
        return;
    }

    const FString BuildProjectName = !ProjectBuild.Name.IsEmpty()
        ? ProjectBuild.Name : CurrentWorldName;
    if (BuildProjectName.IsEmpty())
    {
        CompleteProjectArchiveBuild(false, FString(), TEXT("project build name is empty"));
        return;
    }

    const EV3DSimulatorProjectType BuildProjectType = ProjectBuild.Type;
    const bool bWorldProject = BuildProjectType == EV3DSimulatorProjectType::World;
    FString OutputRootOrPath;
    if (bWorldProject)
    {
        OutputRootOrPath = FSafeFileIO::NormalizeFilePath(
            FPaths::Combine(V3DSimulatorPaths::WorldsRoot(), BuildProjectName));
    }
    else
    {
        OutputRootOrPath = V3DSimulatorProjectWorkspace::AssetArchivePath(BuildProjectName);
        if (OutputRootOrPath.IsEmpty())
        {
            CompleteProjectArchiveBuild(false, FString(), TEXT("project .v3d output path is invalid"));
            return;
        }
    }

    ProjectBuild.bCommitInFlight = true;
    ProjectBuild.Progress = 0.96f;
    LastSaveMessage = bWorldProject
        ? TEXT("Writing and verifying the world .v3d archive...")
        : TEXT("Writing and verifying the project .v3d archive...");
    OnProjectBuildProgress.Broadcast(ProjectBuild.Progress);
    NotifyStateChanged();
    const FString ArchiveConfigJson = ProjectBuild.ConfigJson;
    TArray<FGWorldBuildModel> BuildModels = MoveTemp(ProjectBuild.CompletedModelsData);
    TArray<FSoundDefinition> SoundDefinitions = MoveTemp(ProjectBuild.PendingSounds);
    TMap<FGuid, FString> SoundDefinitionJson = MoveTemp(ProjectBuild.PendingSoundJson);
    TWeakObjectPtr<UGameManagerSubSystem> WeakThis(this);
    const uint64 BuildGeneration = ProjectBuild.Generation;
    const TSharedPtr<TAtomic<bool>, ESPMode::ThreadSafe> Cancellation = ProjectBuild.Cancellation;
    const bool bQueued = FSafeFileIO::RunTrackedWorker(
            ...
            SoundDefinitionJson = MoveTemp(SoundDefinitionJson)]() mutable
    {
                ...
                ArchivePath = MoveTemp(ArchivePath), Error = MoveTemp(Error)]()
        {
            if (UGameManagerSubSystem* StrongThis = WeakThis.Get())
            {
                if (StrongThis->ProjectBuild.Generation == BuildGeneration)
                {
                    StrongThis->CompleteProjectArchiveBuild(bSuccess, ArchivePath, Error);
                }
            }
        });
    });
...
void UGameManagerSubSystem::CompleteProjectArchiveBuild(
    const bool bArchiveSuccess,
    const FString& ArchivePath,
    const FString& Error)
{
    check(IsInGameThread());
    if (!ProjectBuild.bInProgress) return;
    if (UWorld* World = GetWorld())
    {
        World->GetTimerManager().ClearTimer(ProjectBuildProgressTimerHandle);
    }

    const EV3DSimulatorProjectType CompletedProjectType = ProjectBuild.Type;
    const int32 CompletedModelCount = ProjectBuild.TotalModels;
    const TCHAR* ArchiveKind = CompletedProjectType == EV3DSimulatorProjectType::World
        ? TEXT(".v3d") : TEXT(".v3d");
    const FString Message = bArchiveSuccess
        ? FString::Printf(TEXT("%s build completed: %d model(s) (%s)"),
            ArchiveKind, CompletedModelCount, *ArchivePath)
        : FString::Printf(TEXT("%s build failed: %s"),
            ArchiveKind, Error.IsEmpty() ? TEXT("archive build failed") : *Error);

    ActiveProjectBuildTask = nullptr;
    ProjectBuild.Reset();
    ProjectBuild.Progress = 1.0f;
    LastSaveMessage = Message;
    OnProjectBuildProgress.Broadcast(1.0f);
    OnProjectBuildCompleted.Broadcast(bArchiveSuccess, Message);
    NotifyStateChanged();
}

void UGameManagerSubSystem::CancelProjectBuild()
{
    if (ProjectBuild.Cancellation.IsValid())
    {
        ProjectBuild.Cancellation->Store(true);
    }
    ++ProjectBuild.Generation;
    if (UWorld* World = GetWorld())
    {
    ...
    if (IsValid(ActiveProjectBuildTask))
    {
        ActiveProjectBuildTask->OnFinished.RemoveAll(this);
        ActiveProjectBuildTask->Cancel();
    }
    ActiveProjectBuildTask = nullptr;
    ProjectBuild.Reset();
}
