...
int32 UGameUpdateSubSystem::RegisterUpdate(UObject* Owner, TFunction<void(float)>&& UpdateFunction, int32 Priority)
{
    ...
    Entry.Owner = Owner;
    Entry.UpdateFunction = MakeShared<TFunction<void(float)>>(MoveTemp(UpdateFunction));
...
void UGameUpdateSubSystem::Tick(float DeltaTime)
{
    ...
    if (!bInitialized || bIsDispatching)
    {
        return;
    }
    ...
    const uint64 DispatchGeneration = LifecycleGeneration;
    for (const int32 Handle : SortedHandles)
    {
        ...
        if (!Entry || !Entry->Owner.IsValid() || !Entry->UpdateFunction.IsValid())
        {
            PendingRemoveHandleSet.Add(Handle);
            continue;
        }

        // Keep the callable stable when registration reallocates the map or teardown clears it.
        const TSharedPtr<TFunction<void(float)>> Callback = Entry->UpdateFunction;
        (*Callback)(DeltaTime);
        if (!bInitialized || LifecycleGeneration != DispatchGeneration)
        {
            return; // SortedHandles may have been cleared by world/game-instance teardown.
        }
    }
...
void UGameUpdateSubSystem::RebuildDispatchOrderIfNeeded()
{
    ...
    for (const TPair<int32, FGameUpdateEntry>& Pair : UpdateEntries)
    {
        if (Pair.Value.Owner.IsValid() && Pair.Value.UpdateFunction.IsValid())
        {
