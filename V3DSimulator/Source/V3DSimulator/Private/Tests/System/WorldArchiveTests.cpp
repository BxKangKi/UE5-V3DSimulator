...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FWorldFolderNameValidationTest,
    "V3DSimulator.World.WorldFolderNameValidation",
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FRecursiveWorldSourceDiscoveryTest,
    "V3DSimulator.Streaming.RecursiveWorldSourceDiscovery",
    EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FRecursiveWorldSourceDiscoveryTest::RunTest(const FString& Parameters)
{
    ...
    const FString ProjectRoot = FPaths::Combine(Root, TEXT("Project"));
    const FString NestedA = FPaths::Combine(ProjectRoot, TEXT("model"), TEXT("city"));
    const FString NestedB = FPaths::Combine(ProjectRoot, TEXT("props"), TEXT("vehicles"), TEXT("land"));
    ...
    const FString DirectGlb = FPaths::Combine(ProjectRoot, TEXT("direct.glb"));
    ...
    TestTrue(TEXT("Recursively discover every GLB and create missing definitions"),
        ModelDefinitionJson::EnsureDefinitions(
            ProjectRoot, EV3DSimulatorProjectType::World, &CreatedJsonFiles, &DiscoveredGlbFiles));
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FGWorldArchiveRoundTripTest,
    "V3DSimulator.Streaming.V3dRoundTrip",
    EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
...
bool FGWorldArchiveRoundTripTest::RunTest(const FString& Parameters)
{
    ...
    const FString ModelDirectory = FPaths::Combine(
        Sandbox, TEXT("Projects"), TEXT("TestWorld"), TEXT("assets"), TEXT("model"));
    ...
    // materialize every model member merely because they share one .v3d container.
    ...
    const FString SoundDirectory = FPaths::Combine(
        Sandbox, TEXT("Projects"), TEXT("TestWorld"), TEXT("assets"), TEXT("audio"));
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FEntityArchiveRecoveryTest,
    "V3DSimulator.Streaming.EntityCommitRecovery",
    EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
