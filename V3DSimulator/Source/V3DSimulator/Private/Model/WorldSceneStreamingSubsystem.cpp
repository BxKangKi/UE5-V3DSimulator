    ...
    const bool bInRenderOnlyStreaming)
{
    ...
    {
        // Runtime streaming has exactly one legal input. Falling back to the project authoring tree here would
        // violate the archive/source separation and could silently ship an unbounded file loader.
        bStartupFailed = true;
        WriteLogAsync(TEXT("Streaming startup rejected: a verified .v3d archive is not open"));
        return;
    }
...
bool UWorldSceneStreamingSubsystem::ResolveInitialCharacterIndex()
{
    ...
    WriteLogAsync(FString::Printf(
        TEXT("Selected character was not found in .v3d; using the default pawn. Selector=%s"),
        *InitialPlayerName));
    return false;
}
...
void UWorldSceneStreamingSubsystem::HandlePlayerLoadFailure(const FString& Reason)
{
    ...
    {
        // .v3d is immutable for this session, so a failing member cannot become healthy due to
...
{
    FSimulatorFileServices::WriteLogAsync(TEXT("WorldSceneStreamingSubsystem"), Message);
}
