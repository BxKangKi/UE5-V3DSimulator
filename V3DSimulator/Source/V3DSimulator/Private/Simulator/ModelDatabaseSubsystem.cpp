...
namespace ModelDatabasePrivate
{
    ...
    struct FDatabaseBuildResult
    {
        ...
        TArray<TSharedPtr<FGWorldArchiveReader, ESPMode::ThreadSafe>> ProjectArchiveReaders;
        FString Error;
    };
    ...
    void LoadBuiltArchive(const FString& RequestedRoot, FDatabaseBuildResult& Result)
    {
        ...
        if (Records.IsEmpty() && SoundRecords.IsEmpty())
        {
            ...
            Result.Error = TEXT("the .v3d archive contains no assets");
            return;
        }
        ...
        for (FGWorldSoundRecord& Record : SoundRecords)
        {
            ...
            {
                Result.Error = FString::Printf(TEXT("duplicate asset UUID in .v3d archive: %s"), *UUID.ToString());
                Result.ArchiveReader.Reset();
                return;
            }
        ...
        FString& OutError)
    {
        ...
        return V3DSimulatorProjectConfig::Parse(
            Parsed.JsonObject, FallbackName, OutConfig, OutError);
    }

    void LoadAllowedProjectArchives(FDatabaseBuildResult& Result)
    {
        if (!Result.ArchiveReader.IsValid()) return;

        FV3DSimulatorProjectConfig WorldConfig;
        FString ConfigError;
                ...
                ConfigError))
        {
            UE_LOG(LogTemp, Warning,
                TEXT("Resource archive discovery skipped because the world archive config is invalid: %s"),
                *ConfigError);
            return;
        }
        if (WorldConfig.ProjectType != EV3DSimulatorProjectType::World
            || !WorldConfig.bAllowProjectAssets)
        {
            return;
        }

        const FString ResourcesRoot = V3DSimulatorPaths::ResourcesRoot();
        if (ResourcesRoot.IsEmpty() || !IFileManager::Get().DirectoryExists(*ResourcesRoot)) return;

        TArray<FString> ArchiveFiles;
        IFileManager::Get().FindFiles(ArchiveFiles, *(FPaths::Combine(ResourcesRoot, TEXT("*.v3d"))), true, false);
        ArchiveFiles.Sort();
        if (ArchiveFiles.Num() > 512)
        {
            UE_LOG(LogTemp, Warning,
                TEXT("Resource archive discovery found %d packs; only the first 512 sorted archives are considered."),
                ArchiveFiles.Num());
        ...
        {
            const FString CandidatePath = FSafeFileIO::NormalizeFilePath(FPaths::Combine(ResourcesRoot, ArchiveFile));
            ...
            {
                UE_LOG(LogTemp, Warning,
                    TEXT("Resource asset archive ignored. Path=%s Reason=%s"),
                    *CandidatePath, *OpenError);
                continue;
            }

            FV3DSimulatorProjectConfig PackConfig;
            FString PackConfigError;
            if (!ParseArchiveProjectConfig(Reader, FPaths::GetBaseFilename(ArchiveFile), PackConfig, PackConfigError)
                || !V3DSimulatorProjectBuildPolicy::IsAssetProject(PackConfig.ProjectType))
            {
                UE_LOG(LogTemp, Warning,
                    TEXT("Resource asset archive ignored because embedded config is not an asset ProjectType. Path=%s Reason=%s"),
            ...
            if (Records.IsEmpty() && SoundRecords.IsEmpty()) continue;
            ...
            for (const FGWorldModelRecord& Record : Records)
            {
                if (!Record.Definition.UUID.IsValid()
                    || !V3DSimulatorProjectBuildPolicy::AllowsModelType(PackConfig.ProjectType, Record.Definition.ModelType))
                {
                    bPackValid = false;
                    RejectReason = TEXT("model type does not match ProjectType");
                    break;
                }
                if (Result.Definitions.Contains(Record.Definition.UUID))
                {
                    bPackValid = false;
                    RejectReason = FString::Printf(TEXT("model UUID %s collides with an already mounted asset"),
                        *Record.Definition.UUID.ToString(EGuidFormats::DigitsWithHyphensLower));
                    break;
                }
            }
            if (bPackValid)
            {
                ...
                {
                    if (!Record.Definition.UUID.IsValid() || Result.SoundDefinitions.Contains(Record.Definition.UUID))
                    {
                        bPackValid = false;
                        RejectReason = TEXT("invalid or duplicate Sound UUID");
                        break;
                    }
                }
            }
            if (!bPackValid)
            {
                UE_LOG(LogTemp, Warning, TEXT("Resource asset archive ignored. Path=%s Reason=%s"),
            ...
            Result.ProjectArchiveReaders.Add(Reader);
            UE_LOG(LogTemp, Display,
                TEXT("Mounted resource asset archive: %s (%d model(s), %d sound(s), type=%s)"),
                *CandidatePath, Records.Num(), SoundRecords.Num(), *V3DSimulatorProjectTypes::ToString(PackConfig.ProjectType));
        }
    }
    ...
    {
        // Projects/<Project>/resources is the only authoring asset root. config.json stays one level
        // above it and is never mixed into the recursive asset scan.
        if (Cancellation->Load()) return;
        const FString ProjectRoot = RequestedRoot;
        if (!IFileManager::Get().DirectoryExists(*ProjectRoot))
        {
            Result.Error = FString::Printf(TEXT("Project directory does not exist: %s"), *ProjectRoot);
            return;
        }

        const FString FolderName = FPaths::GetCleanFilename(ProjectRoot);
        const FString ModelRoot = V3DSimulatorProjectWorkspace::ProjectResourcesRoot(FolderName);
        if (!IFileManager::Get().DirectoryExists(*ModelRoot))
        {
            Result.Error = FString::Printf(TEXT("Project resources directory does not exist: %s"), *ModelRoot);
            return;
        }
        ...
        if (!V3DSimulatorProjectConfig::Load(
                FPaths::Combine(ProjectRoot, TEXT("config.json")), FolderName,
                ProjectConfig, nullptr, ProjectConfigError))
        {
            Result.Error = FString::Printf(TEXT("Project config is invalid: %s"), *ProjectConfigError);
            return;
        }
        ...
        // Discovery and canonical JSON normalization use recursive scans within the project resources
        // tree only. Runtime mode never enters this function and never touches authoring resources.
        TArray<FString> GlbFiles;
        TArray<FString> UpdatedDefinitions;
        if (!ModelDefinitionJson::EnsureDefinitions(
                ModelRoot, ProjectConfig.ProjectType, &UpdatedDefinitions, &GlbFiles))
        {
            Result.Error = FString::Printf(
        ...
        TArray<FString> UpdatedSoundDefinitions;
        if (!SoundDefinitionJson::EnsureDefinitions(ModelRoot, &UpdatedSoundDefinitions, &WavFiles))
        {
        ...
        {
            Result.Error = TEXT("Authoring asset count exceeds the .v3d safety limit");
            return;
        }
        if (Cancellation->Load()) return;
        GlbFiles.Sort();
        ...
        UE_LOG(LogTemp, Display,
            TEXT("Project discovery found %d GLB file(s) and %d WAV file(s) recursively under '%s' (%d asset JSON file(s) normalized)."),
            GlbFiles.Num(), WavFiles.Num(), *ModelRoot, UpdatedDefinitions.Num() + UpdatedSoundDefinitions.Num());
        for (const FString& GlbPath : GlbFiles)
        {
            UE_LOG(LogTemp, Verbose, TEXT("Project source GLB: %s"), *GlbPath);
        }
        ...
        for (int32 Index = 0; Index < GlbFiles.Num(); ++Index)
        {
            ...
            if (!V3DSimulatorProjectBuildPolicy::AllowsModelType(ProjectConfig.ProjectType, Item.Definition.ModelType))
            {
                const FString Message = FString::Printf(
                    TEXT("Model definition rejected by ProjectType policy. ProjectType=%s ModelType=%s JSON=%s"),
                    *V3DSimulatorProjectTypes::ToString(ProjectConfig.ProjectType),
                    *V3DSimulatorModelTypes::ToString(Item.Definition.ModelType), *JsonPath);
                UE_LOG(LogTemp, Error, TEXT("%s"), *Message);
                AppendError(Result.Error, Message);
                continue;
            }
    ...
    FModelDatabaseReady Completion)
{
        ...
        [WeakThis, RequestedRoot, RequestedGeneration, Completion, Cancellation]() mutable
    {
        ...
        {
            LoadAllowedProjectArchives(Result);
        }
        if (Cancellation->Load()) return;
            ...
            [WeakThis, RequestedRoot, RequestedGeneration, Completion, Result = MoveTemp(Result)]() mutable
        {
            ...
            StrongThis->ProjectArchiveReaders = MoveTemp(Result.ProjectArchiveReaders);
    ...
    FModelDatabaseReady Completion)
{
        ...
        [WeakThis, RequestedRoot, RequestedGeneration, Completion, Cancellation]() mutable
    {
            ...
            [WeakThis, RequestedRoot, RequestedGeneration, Completion, Result = MoveTemp(Result)]() mutable
        {
            ...
            StrongThis->ProjectArchiveReaders.Reset();
